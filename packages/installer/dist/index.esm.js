import { _exists, _mkdir, MinecraftFolder, _writeFile, _readFile, checksum, _pipeline, Version, LibraryInfo, getPlatform, diagnoseFile } from '@xmcl/core';
import 'crypto';
...
async function getMetadata(srcUrl, _headers, agents, useGet = false) {
    ...
    const res = await fetch$1(srcUrl, {
    ...
    async processDownload(metadata, abortSignal) {
        ...
        await Promise.all(this.segments.map(async (segment, index) => {
            ...
            try {
                ...
                await _pipeline(response, fileStream);
