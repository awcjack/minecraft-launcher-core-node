import { _exists, _mkdir, MinecraftFolder, _writeFile, _readFile, checksum, Version, LibraryInfo, _pipeline, getPlatform, diagnoseFile } from '@xmcl/core';
import 'crypto';
import { request, Agent } from 'http';
import { unlink as unlink$1, stat as stat$1, link as link$1, open as open$1, close as close$1, copyFile as copyFile$1, ftruncate, createReadStream, fstat, fdatasync, createWriteStream } from 'fs';
import { request as request$1, Agent as Agent$1 } from 'https';
import { URL, fileURLToPath } from 'url';
import { spawn, exec } from 'child_process';
import { dirname, join, delimiter, basename, resolve } from 'path';
import { promisify } from 'util';
import { task, AbortableTask, CancelledError, BaseTask, TaskState, TaskGroup } from '@xmcl/task';
import { parse } from '@xmcl/forge-site-parser';
...
const link = promisify(link$1);
const open = promisify(open$1);
const close = promisify(close$1);
const copyFile = promisify(copyFile$1);
const truncate = promisify(ftruncate);
function missing(target) {
    return _exists(target).then((v) => !v);
}
    ...
    return new Promise((resolve, reject) => {
        function follow(options) {
            if (!isValidProtocol(options.protocol)) {
                ...
                let [req, agent] = options.protocol === "http:" ? [request, agents.http] : [request$1, agents.https];
...
async function fetchText(url, agent) {
...
async function getIfUpdate(url, timestamp, agent = {}) {
    let parsed = new URL(url);
    if (!isValidProtocol(parsed.protocol)) {
        throw new Error(`Invalid protocol ${parsed.protocol}`);
    }
...
async function getLastModified(url, timestamp, agent = {}) {
    let parsed = new URL(url);
    if (!isValidProtocol(parsed.protocol)) {
        throw new Error(`Invalid protocol ${parsed.protocol}`);
    }
...
async function installFabricYarnAndLoader(yarnVersion, loaderVersion, minecraft, options = {}) {
    ...
    await _writeFile(jsonFile, JSON.stringify(body));
    return id;
}
/**
...
async function installFabric(loader, minecraft, options = {}) {
    ...
    await ensureFile(jsonFile);
    await _writeFile(jsonFile, JSON.stringify({
        id,
        inheritsFrom,
        mainClass,
        libraries,
...
function installLiteloaderTask(versionMeta, location, options = {}) {
    return task("installLiteloader", async function installLiteloader() {
        const mc = MinecraftFolder.from(location);
        ...
        const mountedJSON = await this.yield(task("resolveVersionJson", async function resolveVersionJson() {
            ...
            return _readFile(mc.getVersionJson(mountVersion)).then((b) => b.toString()).then(JSON.parse);
        }));
        const versionInf = await this.yield(task("generateLiteloaderJson", async function generateLiteloaderJson() {
            ...
            await _writeFile(join(versionPath, inf.id + ".json"), JSON.stringify(inf, undefined, 4));
            return inf;
        }));
...
function createAgents(options = {}) {
    ...
    return {
        http: new Agent({
            maxSockets: (_a = options.maxSocket) !== null && _a !== void 0 ? _a : cpus().length * 4,
            maxFreeSockets: (_b = options.maxFreeSocket) !== null && _b !== void 0 ? _b : 64,
            keepAlive: true,
        }),
        https: new Agent$1({
            maxSockets: (_c = options.maxSocket) !== null && _c !== void 0 ? _c : cpus().length * 4,
...
async function getMetadata(srcUrl, _headers, agents, useGet = false) {
    ...
    const res = await fetch$1(srcUrl.toString(), {
...
var AbortController = unwrapExports(cjsPonyfill);
cjsPonyfill.AbortController;
cjsPonyfill.AbortSignal;
cjsPonyfill.abortableFetch;
...
class Download {
    ...
    */
    ...
    async processDownload(metadata, abortSignal) {
        ...
        await Promise.all(this.segments.map(async (segment, index) => {
            ...
            try {
                ...
                const res = await fetch$1((_a = metadata.url) === null || _a === void 0 ? void 0 : _a.toString(), {
                    method: "GET",
                    headers: {
                        ...this.headers,
                        Range: `bytes=${segment.start}-${(_b = (segment.end)) !== null && _b !== void 0 ? _b : ""}`,
                    },
    ...
    async downloadUrl(url, abortSignal) {
        let attempt = 0;
...
class DownloadTask extends AbortableTask {
    constructor(options) {
        super();
        this.abort = () => { };
...
function installAssetsTask(version, options = {}) {
    return task("assets", async function () {
        ...
        const { objects } = JSON.parse(await _readFile(jsonPath).then((b) => b.toString()));
...
class InstallJarTask extends DownloadTask {
    constructor(version, minecraft, options) {
        ...
        const destination = join(folder.getVersionRoot(version.id), type === "client" ? version.id + ".jar" : version.id + "-" + type + ".jar");
...
function installByProfileTask(installProfile, minecraft, options = {}) {
    return task("installByProfile", async function () {
        ...
        const versionJson = await _readFile(minecraftFolder.getVersionJson(installProfile.version)).then((b) => b.toString()).then(JSON.parse);
        const libraries = Version.resolveLibraries([...installProfile.libraries, ...versionJson.libraries]);
 ...
 * @throws {@link PostProcessError}
 */
class PostProcessingTask extends AbortableTask {
    ...
    async findMainClass(lib) {
        ...
        let mainClass;
        try {
            zip = await open$2(lib, { lazyEntries: true });
...
class DownloadForgeInstallerTask extends DownloadTask {
    constructor(forgeVersion, installer, minecraft, options) {
        ...
        const library = Version.resolveLibrary({
...
function extractEntryTo(zip, e, dest) {
    return openEntryReadStream(zip, e).then((stream) => _pipeline(stream, createWriteStream(dest)));
}
async function installLegacyForgeFromZip(zip, entries, profile, mc, options) {
    ...
    await Promise.all([
        _writeFile(versionJsonPath, JSON.stringify(versionJson, undefined, 4)),
...
async function unpackForgeInstaller(zip, entries, forgeVersion, profile, mc, jarPath, options) {
    const versionJson = await readEntry(zip, entries.versionJson).then((b) => b.toString()).then(JSON.parse);
    // apply override for inheritsFrom
    versionJson.id = options.versionId || versionJson.id;
    versionJson.inheritsFrom = options.inheritsFrom || versionJson.inheritsFrom;
    ...
    promises.push(_writeFile(installJsonPath, JSON.stringify(profile)), _writeFile(versionJsonPath, JSON.stringify(versionJson)));
...
function installByInstallerTask(version, minecraft, options) {
    return task("installForge", async function () {
        ...
        return withAgents(options, async (options) => {
            ...
            const zip = await open$2(jarPath, { lazyEntries: true, autoClose: false });
            ...
            const profile = await readEntry(zip, entries.installProfileJson).then((b) => b.toString()).then(JSON.parse);
...
async function getForgeVersionList(option = {}) {
    ...
    return getAndParseIfUpdate(url, parse, option.original);
}

function getDefaultEntryResolver() {
    return (e) => e.fileName;
}
class UnzipTask extends BaseTask {
    ...
    async handleEntry(entry, relativePath) {
        ...
        await _pipeline(readStream, writeStream);
    }
    async runTask() {
        const promises = [];
...
class DownloadCurseforgeFilesTask extends TaskGroup {
    ...
    async runTask() {
        ...
        const resolver = ((_b = this.options) === null || _b === void 0 ? void 0 : _b.filePathResolver) || ((p, f, m, u) => m.getMod(basename(u)));
...
async function normalizeInput(input) {
    if (typeof input === "string" || input instanceof Buffer) {
        const zip = await open$2(input, { lazyEntries: true, autoClose: false });
...
function installOptifineTask(installer, minecraft, options = {}) {
    return task("installOptifine", async function () {
        ...
        const zip = await open$2(installer);
        ...
        await this.yield(task("json", async () => {
            await ensureFile(versionJSONPath);
            await _writeFile(versionJSONPath, JSON.stringify(versionJSON, null, 4));
        }));
        ...
        if (launchWrapperEntry) {
            await this.yield(task("library", async () => {
                ...
                await _writeFile(wrapperDest, await readEntry(zip, launchWrapperEntry));
            }));
        }
        // write the optifine
...
function installJreFromMojangTask(options) {
    ...
    return task("installJreFromMojang", async function () {
        const info = await this.yield(task("fetchInfo", () => fetchJson("https://launchermeta.mojang.com/mc/launcher.json")));
...
async function fetchJavaRuntimeManifest(options = {}) {
    ...
    const platform = (_c = options.platform) !== null && _c !== void 0 ? _c : getPlatform();
    const runtimeTarget = (_d = options.target) !== null && _d !== void 0 ? _d : JavaRuntimeTargetType.Beta;
    const resolveTarget = () => {
