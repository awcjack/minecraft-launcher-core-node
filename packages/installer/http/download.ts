...
import { close, copyFile, ensureFile, exists, open, truncate, unlink } from "../utils";
...
import { ChecksumValidatorOptions, resolveValidator, Validator } from './validator';
...
export class DownloadError extends Error {
    constructor(
        readonly reason: 'abort' | 'exception',
        readonly metadata: ResourceMetadata | undefined,
        readonly headers: Record<string, any>,
        readonly destination: string,
        readonly segments: Segment[],
        readonly errors: any[]
    ) { super("The download is aborted!"); }
}
...
export class Download {
    ...
    protected async processDownload(metadata: ResourceMetadata, abortSignal: AbortSignal): Promise<void> {
        ...
        await Promise.all(this.segments.map(async (segment) => {
            ...
            try {
                if (abortSignal.aborted || flag) { return Promise.reject(Aborted) }
                ...
                await Promise.all([
                    ...
                    new Promise<void>((resolve, reject) => {
                        ...
                        // unpipe means the response is aborted
                        fileStream.on("unpipe", () => resolve());
                        // finish means the all data are property transferred
        ...
        if (flag) {
            throw new DownloadError(flag === 1 ? 'abort' : 'exception',
                this.metadata,
                this.headers,
                this.destination,
                this.segments,
    ...
    protected async downloadUrl(url: string, abortSignal: AbortSignal) {
        ...
        while (true) {
            try {
                ...
                // common error means it might due to the bad network condition
    ...
    async start(abortSignal: AbortSignal = resolveAbortSignal()) {
        try {
            ...
            if ((await pfstat(this.fd)).size !== 0
                && !await this.validator.validate(this.fd, this.destination, this.urls[0]).catch(e => e)) {
            ...
            if (aggregatedErrors.length > 0) {
                throw aggregatedErrors[aggregatedErrors.length - 1]
            }
        } catch (e) {
            if (!(e instanceof DownloadError) || !(e instanceof ValidationError)) {
